...
function SidebarProfile(p) {
    ...
    return (
        <>
            {/* mobile */}
            <div className="flex flex-col xl:hidden w-full gap-3 p-3 pb-0 pr-0 text-zinc-100">
                {/* user */}
                <div className="flex flex-row w-full p-6 gap-6 items-center text-2xl rounded-3xl bg-zinc-900">
                    ...
                    <div className="grid grid-row-2 w-full gap-3 overflow-hidden">
                        <div className="flex flex-col w-full gap-3">
                            ...
                            </p>
                            {
                                ...
                                <button className={`${ isFollowed ? "text-zinc-400 border-2 border-orange-500 hover:border-orange-400" : "bg-orange-500 hover:bg-orange-400" } w-fit h-fit py-1 px-3 sm:px-6 rounded-3xl text-sm sm:text-lg text-center font-semibold gap-4 overflow-hidden`} onClick={ () => { handleFollowUser() } }>
                                    {
                                        isFollowed ?
                                        "Unfollow"
                                        :
                ...
                <div className="flex flex-col gap-3 p-3 pb-0 rounded-3xl bg-zinc-900">
                    ...
                    <div className="flex gap-3 overflow-x-scroll scrollable-div">
                        {
                            followers &&
                            followers.length > 0 &&
            ...
            {/* desktop */}
            <div className="hidden xl:grid pl-3 w-full h-full overflow-hidden" style={ { gridTemplateColumns: "repeat(15, minmax(0, 1fr))" } }>
                <div className="flex flex-col gap-3 text-zinc-100 col-span-4 pointer-events-auto overflow-y-scroll overflow-x-hidden scrollable-div">
                    {/* user */}
                        ...
                        <div className="flex flex-col gap-3">
                            {
                                ...
                                followers.length > 0 &&
                                followers.map((follower, index) => 
                                    <Link to={`/${ follower.username }`} className="flex flex-row items-center p-3 gap-6 text-xl rounded-3xl hover:bg-zinc-500" key={ index }>
                                        <img className="w-16 h-16 aspect-1 rounded-full object-cover" src={ follower.profilePicture || ProfileIcon } alt="" />
                                        <p>{ follower.username }</p>
                                    </Link>
                                )
