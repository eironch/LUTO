...
function Home({
    ...
    function fetchFeedRecipes(filters) {
        ...
        axios.get(`${ process.env.REACT_APP_API_URL || 'http://localhost:8080' }/feed-recipes`, { params: { userId: user.userId, filters, sort: user.accountType === "user" ? { createdAt: -1 } : { flagCount: 1 }, fetchedRecipeIds }})
            .then(res => {
    ...
    return (
        <div className="pr-3 hide-scrollbar xl:scrollable-div overflow-y-scroll " ref={ scrollDivRef }>
            {/* navbar */}
            {
                screenSize > 3 &&
            ...
            <div className="flex flex-col pr-0 gap-3 h-dvh">
                <div className="flex flex-col gap-3 p-3 pt-0 pr-0 pb-20 xl:pb-0">
                    ...
                    <div className="flex xl:grid w-full gap-3 h-full" style={ { gridTemplateColumns: "repeat(15, minmax(0, 1fr))" } }>
                        ...
                        <div className="col-span-11 w-full mb-0 xl:-mb-3">
                            { 
                                feedRecipes &&
